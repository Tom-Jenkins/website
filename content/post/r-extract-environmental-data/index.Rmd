...
[Bio-ORACLE](https://www.bio-oracle.org/) is website that allow users to download biotic, geophysical and environmental data for surface and benthic marine realms in raster format. All data layers are available globally at the same spatial resolution of 5 arcmin (approximately 9.2 km at the equator). At the time of writing, Bio-ORACLE also allows future layers of four variables to be downloaded: temperature, salinity, current velocity and ice thickness. A list of all available data sets for surface and benthic layers can be found [here](https://www.bio-oracle.org/downloads-to-email.php).
...
#### Export a csv file containing variables of interest
...
```{r}
# Select layers that are not correlated (-0.6 > x < 0.6)
...
> The version of the `layers_correlation()` function used in this post currently does not accept version 2.1 of the layers (e.g. BO21_tempmean_bdmean) so version 2.0 was used to illustrate the code above.
...
Create or import a data containing longitude and latitude points. In this example, we will create 100 random points directly from the rasters.
```{r}
set.seed(123)
...
# Violin plot and raw point data
marine.data %>% 
